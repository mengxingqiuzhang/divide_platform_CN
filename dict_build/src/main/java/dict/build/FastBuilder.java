	...
	public String genLeft(String rawTextFile, int maxLen, int memSize) {
		...
		try (BufferedReader breader = Files.newReader(rawFile, Charsets.UTF_8);
			...
			try(BufferedReader nsr = Files.newReader(ngramSort, Charsets.UTF_8)) {
				...
				/**
				* 逐句读ngramSort 依旧是倒序
				* Map的键值对为<string, countermap>，countermap为一个map的封装，其key为string，值为一个int（计数）
				* 将长度为i的前缀作为key
				*/
				while (null != (curr = nsr.readLine())) {
					if (null == first) {
						for (int i = 1; i < curr.length(); ++i) {
							...
							//将前缀放入Map中作为键，一个前缀键对应一个容器，容器内又拥有很多键值对
							//如果Map中包含了该前缀键，就在对应容器中对suffix使用incr函数
							//函数incr(key)表示，如果容器内key，则对应值+1；否则创建新的<key,1>键值对
							//first用于记录当前行的首字母
						...
						if (!curr.startsWith(first)) {
							...
							//对map中的每一个键值w进行遍历，取出他们的容器countermap，countall返回内部的map对象
							//freq将容器中的所有键对应的值都加起来
